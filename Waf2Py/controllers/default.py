...
def ManageAllRules():
    ...
    try:
        ...
        NewLogError(db2, auth.user.username, "Manage all rules: Invalid characters in data received - Debug: %s" %(b))
...
def ManageRules():
    ...
    try:
        ...
        NewLogError(db2, auth.user.username, "Manage rules: Invalid characters in data received - Debug: %s %s %s" %(b,c,d))
        ...
        if query:
            ...
            for x in f:
                ...
                if request.vars['rule'] == x:
                    ...
                    NewLogApp(db2, auth.user.username, "Manage rules: Rule: "+request.vars['rule'] + " "+ request.vars['action'])    
                    msg = 'Done'
                    break
...
def ProdEdit():
    ...
    if b == 'YES':
        ...
        if query[0]['app_name']:
            ...
            f = open('/opt/waf/nginx/etc/modsec_rules/'+query[0]['app_name']+'/rules.list', 'a')
            f.write(rules_that_exist)
            f.close()

            for i in rules_that_exist.splitlines():
                #Append all rules except the INITIALIZATION
                if "INITIALIZATION" == i:
                    pass
                else:
                    rule_list.append(i)
